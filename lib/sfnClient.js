...
async function stopExecution(stateMachineArn, executionName, input) {
	const command = new StopExecutionCommand({ stateMachineArn, name: executionName, input: JSON.stringify(input) });
	return await client.send(command);
}
